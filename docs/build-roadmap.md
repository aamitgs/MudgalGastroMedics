...
| # | Item | Current state | Build | Pri | Effort | Depends on |
|---|---|---|---|---|---|---|
| 3.1 | **Shared enterprise DataTable** | ✅ Primitive built. ✅ **Patients**, ✅ **Appointments**, ✅ **Laboratory**, ✅ **OPD Queue**, ✅ **Pharmacy**, ✅ **IPD & Beds**, ✅ **Inventory**, ✅ **HR**, ✅ **Finance**, ✅ **Procedures**, ✅ **Communication**, ✅ **CMS**, ✅ **Doctor Workflow**, ✅ **Billing Summary**, ✅ **AI Reviews**, ✅ **Audit Log**, ✅ **Automation** adopted (`lib/*-query.ts` per module) — **17 of 23 admin modules**, every one that is genuinely a searchable/sortable list of records. **Rollout audit complete**: the remaining 6 were individually read and evaluated, not just skipped by assumption — all are single-object dashboards, fixed reference data, or explicitly security-deferred, none are a list-of-records fit: **Reports** (one aggregate daily-summary object), **Analytics** (one aggregate snapshot — KPI cards + bar-chart mixes, no record list; real analytics drill-down is tracked separately as 3.5), **Enterprise Modules** (fixed ~30-item catalog browser), **Production Readiness** (fixed ~9-check audit list, read-only, grouped by area — more meaningful as sections than table rows), **Settings** (explicitly "read-only reference for staff": static hospital profile, one stubbed branch, an 8-row role-access reference, a readiness-check subset), **User Management/Access** (the one deferred-not-skipped case — a genuine small list of staff accounts, but every row action is a security-sensitive RBAC operation gated by `disabled={!isSuperAdmin}` and a two-person-rule approval flow; low pagination/volume payoff doesn't justify touching it without being asked). Track 3.1 is functionally done pending a user decision on User Management | Done for all genuine-fit modules. Open question: convert User Management/Access despite its security sensitivity, or leave it as-is — needs a decision, not further autonomous evaluation | **High** | L | — |
