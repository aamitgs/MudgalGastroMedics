...
| # | Item | Current state | Build | Pri | Effort | Depends on |
|---|---|---|---|---|---|---|
| 3.1 | **Shared enterprise DataTable** | ✅ Primitive built. ✅ **Patients**, ✅ **Appointments**, ✅ **Laboratory**, ✅ **OPD Queue**, ✅ **Pharmacy**, ✅ **IPD & Beds**, ✅ **Inventory**, ✅ **HR**, ✅ **Finance**, ✅ **Procedures**, ✅ **Communication**, ✅ **CMS**, ✅ **Doctor Workflow**, ✅ **Billing Summary**, ✅ **AI Reviews** adopted (`lib/*-query.ts` per module). HR, Finance and Billing Summary are client-paginated since another control on the same screen needs the complete unfiltered data regardless. AI Reviews is server-paginated like Lab/Procedures/CMS with a backward-compatible no-param full-list branch on `app/api/ai/reviews`, status + source filters, and a "Review" detail panel below the table for the rich per-case content (summary, flags, preparation checklist, reception script, doctor note) too large for a cell. **Enterprise Modules ("30-module command center") evaluated and skipped for 3.1**: it's a fixed ~30-item catalog browser (card-grid picker + per-module scoped task list), not a searchable/sortable dataset — DataTable doesn't fit; leave as-is. 11 modules still card-stacks | Adopt module-by-module: next candidates are Enterprise/Analytics dashboards (Analytics, Automation, Settings) — evaluate each for genuine tabular fit before converting, per the Enterprise Modules lesson | **High** | L | — |
