...
| # | Item | Current state | Build | Pri | Effort | Depends on |
|---|---|---|---|---|---|---|
| 3.1 | **Shared enterprise DataTable** | ✅ Primitive built. ✅ **Patients**, ✅ **Appointments**, ✅ **Laboratory**, ✅ **OPD Queue**, ✅ **Pharmacy**, ✅ **IPD & Beds**, ✅ **Inventory**, ✅ **HR**, ✅ **Finance**, ✅ **Procedures**, ✅ **Communication**, ✅ **CMS**, ✅ **Doctor Workflow**, ✅ **Billing Summary**, ✅ **AI Reviews**, ✅ **Audit Log** adopted (`lib/*-query.ts` per module). HR, Finance and Billing Summary are client-paginated since another control on the same screen needs the complete unfiltered data regardless. AI Reviews and Audit Log are server-paginated like Lab/Procedures/CMS with a backward-compatible no-param full-list branch on their route (Audit Log's branch also preserves the old caller-supplied `limit` for `scripts/verify-auth-flows.mjs`). Audit Log replaces the old hard `limit=120` fetch — now fetches the store's already-bounded 1000-event retention window and paginates/filters/sorts over that — with a severity filter and a read-only "Details" panel below the table for the field-change diff / metadata / device context too large for a cell. **Enterprise Modules ("30-module command center") evaluated and skipped for 3.1**: it's a fixed ~30-item catalog browser, not a searchable/sortable dataset — DataTable doesn't fit; leave as-is. 10 modules still card-stacks | Adopt module-by-module: next candidates are User Management (Access) or Reports — evaluate each for genuine tabular fit before converting, per the Enterprise Modules lesson | **High** | L | — |
