...
| 1.12 | **Zod convergence on REST routes** | IN PROGRESS, started 2026-07-08. First slice done: `app/api/patients/route.ts` (POST + PATCH) converted from hand-rolled `typeof` guards onto `lib/validation/patients.ts` (`patientCreateSchema`/`patientUpdateSchema`) plus a small shared `lib/validation/http.ts` (`firstZodIssueMessage`, mapping a `ZodError` to the single string the `{ ok, error }` REST shape expects — distinct from the server actions' field-keyed `fieldErrors`, since routes need one message not a per-field map). **Bug caught by live verification before shipping**: the real `AdminPatients.tsx` client sends `forceNew: duplicateMatch && confirmedNewPatient`, which is JS `null` (not `false`) whenever there's no duplicate match — an initial `z.boolean().optional()` schema rejected that with a 400 on every ordinary create, since `.optional()` only allows `undefined`, not `null`. Fixed with `.nullish()`. This is the reason every future slice of this item must include a real browser run against the actual calling component, not just direct API calls with hand-picked payloads — hand-picked test payloads would never have sent `null` and would have shipped this broken. 61 of 62 route files remain (still 0 importing zod before this slice); continue module-by-module, prioritizing routes with the most complex/error-prone hand-rolled validation | Migrate route bodies onto shared Zod schemas module-by-module (P5 "validate everything", one validation culture) | Med | M | — |
