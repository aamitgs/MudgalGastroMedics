...
| # | Item | Current state | Build | Pri | Effort | Depends on |
|---|---|---|---|---|---|---|
| 3.1 | **Shared enterprise DataTable** | ✅ Primitive built (`components/design-system/DataTable.tsx`). ✅ **Patients** and ✅ **Appointments** adopted with server pagination/sort/filter (`lib/patient-query.ts`, `lib/appointment-query.ts` — both backward compatible; Appointments keeps the AI Planning Note as a detail panel below the table). 24 modules still card-stacks | Adopt module-by-module: **OPD Queue or Lab next** (both share the DataTable-friendly shape). Virtualization still deferred — server pagination caps rows sent to the client | **High** | L | — |
