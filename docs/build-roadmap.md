...
| # | Item | Current state | Build | Pri | Effort | Depends on |
|---|---|---|---|---|---|---|
| 3.1 | **Shared enterprise DataTable** | ✅ Primitive built. ✅ **Patients**, ✅ **Appointments**, ✅ **Laboratory**, ✅ **OPD Queue**, ✅ **Pharmacy**, ✅ **IPD & Beds**, ✅ **Inventory** adopted (`lib/*-query.ts` per module). Inventory needed no detail panel — every field (incl. Track 0.8 batch/lot/expiry) fits as a column; gained Low-stock and Expiry-alert quick filters alongside the existing category filter. 19 modules still card-stacks | Adopt module-by-module: next candidates are HR, Finance, or Procedures | **High** | L | — |
