...
| 1.3 | **Adopt the shared `Button`** | PARTIAL (~40%), re-measured 2026-07-07: 63 raw `<button>`s remain (down from 82), spread across 27 files (not the 5 previously logged) — heaviest: `PatientPortalAccess.tsx` (10), `DoctorPortalWorkspace.tsx` (9), `AccessLogin.tsx` (6). `ActionButton` itself is healthy: 25 files, 83 call sites | Migrate to the design-system Button with all variants (primary/secondary/outline/ghost/destructive/success/warning/link/icon) + loading/pressed/disabled/keyboard states | High | M | — |
...
| 1.11 | **Retire HMS/ERP positioning language** | Confirmed still present on 2026-07-07 audit: `app/hms-erp/page.tsx` + `app/platform/page.tsx` still say "HMS"/"ERP"/"Hospital ERP" (title metadata, headings, nav labels), 11 matches across the two files | Reword to "Enterprise Healthcare Platform" language; keep the `/hms-erp` URL as a redirect for SEO continuity | Med | S | — |
