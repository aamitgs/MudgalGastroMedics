...
| # | Item | Current state | Build | Pri | Effort | Depends on |
|---|---|---|---|---|---|---|
| 3.1 | **Shared enterprise DataTable** | ✅ Primitive built (`components/design-system/DataTable.tsx`): sticky header, column pinning/resize/visibility, multi-sort, row selection + bulk actions, CSV export, print, keyboard nav (↑/↓), skeleton/empty/error. ✅ Server pagination/filtering/sorting piloted end-to-end on **Patients** (`lib/patient-query.ts` + `/api/patients`, backward compatible). 25 modules still card-stacks | Adopt module-by-module: **Appointments next**, per the original sequencing. Virtualization deferred — server pagination already caps rows sent to the client, so it's not yet needed; revisit only if a single page size grows large | **High** | L *(was XL — primitive is done)* | — |
