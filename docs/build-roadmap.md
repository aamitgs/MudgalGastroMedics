...
| # | Item | Current state | Build | Pri | Effort | Depends on |
|---|---|---|---|---|---|---|
| 3.1 | **Shared enterprise DataTable** | ✅ Primitive built. ✅ **Patients**, ✅ **Appointments**, ✅ **Laboratory**, ✅ **OPD Queue**, ✅ **Pharmacy**, ✅ **IPD & Beds** adopted. IPD's admissions table paginates *client-side* against the already-loaded list (`lib/ipd-admission-query.ts`) since `BedWardMap` needs the full unpaginated set regardless — documented deviation from the server-paginate default; `BedWardMap` itself untouched. Gained a "Needs attention" filter surfacing HDU escalations (Track 0.2-style). 20 modules still card-stacks | Adopt module-by-module: next candidates are HR, Finance, or Inventory | **High** | L | — |
