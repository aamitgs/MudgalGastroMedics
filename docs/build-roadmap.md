...
| # | Item | Current state | Build | Pri | Effort | Depends on |
|---|---|---|---|---|---|---|
| 3.1 | **Shared enterprise DataTable** | ✅ Primitive built. ✅ **Patients**, ✅ **Appointments**, ✅ **Laboratory**, ✅ **OPD Queue**, ✅ **Pharmacy**, ✅ **IPD & Beds**, ✅ **Inventory**, ✅ **HR**, ✅ **Finance**, ✅ **Procedures**, ✅ **Communication** adopted (`lib/*-query.ts` per module). HR and Finance are each two independent DataTables on one screen, both client-paginated since another control on the same screen needs the complete unfiltered arrays regardless. Procedures and Communication are server-paginated like Lab/OPD/Pharmacy, each with a backward-compatible no-param full-list branch on their API route. Procedures adds a "Manage" detail panel below the table for the 8-toggle safety checklist, doctor/anesthesia fields and findings/complications/notes. Communication replaces the old hard `logs.slice(0, 14)` cap with real pagination, adds status/channel filters, and keeps per-row Call/WhatsApp quick actions as compact link buttons in an actions column. 15 modules still card-stacks | Adopt module-by-module: next candidates are CMS or AI Reviews | **High** | L | — |
