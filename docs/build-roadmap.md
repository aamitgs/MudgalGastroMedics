...
| 1.12 | **Zod convergence on REST routes** | IN PROGRESS, started 2026-07-08. Two slices done. **Patients** (`app/api/patients/route.ts`, POST + PATCH): `lib/validation/patients.ts` (`patientCreateSchema`/`patientUpdateSchema`) plus a shared `lib/validation/http.ts` (`firstZodIssueMessage`, mapping a `ZodError` to the single string the `{ ok, error }` REST shape expects — distinct from the server actions' field-keyed `fieldErrors`). **Bug caught by live verification before shipping**: the real `AdminPatients.tsx` client sends `forceNew: duplicateMatch && confirmedNewPatient`, which is JS `null` (not `false`) whenever there's no duplicate match — an initial `z.boolean().optional()` schema rejected that with a 400 on every ordinary create, since `.optional()` only allows `undefined`, not `null`. Fixed with `.nullish()`. This is why every slice of this item needs a real browser run against the actual calling component(s), not just hand-picked direct API payloads — those would never have sent `null` and would have shipped this broken. **OPD** (`app/api/opd/route.ts`, POST + PATCH): `lib/validation/opd.ts` (`opdVisitCreateSchema`/`opdVisitUpdateSchema`); the field-level authorization split (clinical fields need `prescriptions`, billing fields need `billing`, else `appointments`) still inspects the body *before* calling `authorize()`, now reading from the parsed/typed result instead of raw `typeof` guards — preserves the exact existing order (already required, since which permission to check depends on body content). Verified live against all 5 real callers (`AdminDoctorWorkflow`, `AdminBillingSummary`, `AdminAppointments`, `AdminOpdQueue`, `DoctorPortalWorkspace`) — status/billing dropdowns and clinical-note fields all confirmed working through the browser, not just direct API calls. 60 of 62 route files remain; continue module-by-module — IPD (the 5-branch discriminated union by `type`) is next, and every remaining slice must include a live check of each real caller component, per the `forceNew` lesson above | Migrate route bodies onto shared Zod schemas module-by-module (P5 "validate everything", one validation culture) | Med | M | — |
