...
| 1.3 | **Adopt the shared `Button`** | IN PROGRESS, updated 2026-07-08: **important scope correction** — `ActionButton` is documented as the *staff*-facing button ("the app's own button language... source of truth for staff action buttons") and has never been used on any public page. The original 63-raw-button count wrongly swept in public/patient-facing files (`PatientPortalAccess.tsx`, `PatientHealthDashboard.tsx`, `GalleryGrid.tsx`, `Header.tsx`, `AppointmentForm.tsx`) that are **out of scope** — migrating them would blur the "two products, never mixed" boundary the public site and OS/admin chrome are built on. Done so far: `DoctorPortalWorkspace.tsx` (9 → 2) and `AccessLogin.tsx` (6 → 2, all 4 `buttonClass` submit buttons across the credentials/password-change/MFA-setup/MFA-verify steps now share `ActionButton variant="primary"`; the now-dead `buttonClass` constant removed). In both files, 2 raw buttons remain deliberately unmigrated per file: card/list-item selectors in DoctorPortalWorkspace (don't fit the pill-button shape), and in AccessLogin a bare icon-only password-visibility toggle plus a bare text-link "Forgot password?" toggle (neither has a border/background by design; `ActionButton` has no plain "link" variant — the roadmap's original 1.3 description names one as a target, but adding it would touch a primitive already used in 25 files for one narrow case, not worth the risk yet). Remaining genuinely-staff candidates: `HospitalOperatingSystem.tsx` (4, handle carefully — the one sanctioned monolith), `WorkspaceLauncher.tsx` (3), `StaffChrome.tsx` (3), `AdminPatients.tsx` (3) | Migrate remaining staff-facing raw buttons module-by-module; leave public-facing components on their own button styling | High | M | — |
