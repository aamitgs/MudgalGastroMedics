...
| 1.12 | **Zod convergence on REST routes** | IN PROGRESS, started 2026-07-08. Three slices done. **Patients** (`app/api/patients/route.ts`): `lib/validation/patients.ts` plus a shared `lib/validation/http.ts` (`firstZodIssueMessage`, mapping a `ZodError` to the single string the `{ ok, error }` REST shape expects). **Bug caught by live verification before shipping**: the real `AdminPatients.tsx` client sends `forceNew: duplicateMatch && confirmedNewPatient`, which is JS `null` (not `false`) whenever there's no duplicate match — `.optional()` rejected that with a 400 on every ordinary create; fixed with `.nullish()`. This is why every slice needs a real browser run against the actual calling component(s), not just hand-picked direct API payloads. **OPD** (`app/api/opd/route.ts`): `lib/validation/opd.ts`; the field-level authorization split (clinical → `prescriptions`, billing → `billing`, else `appointments`) still inspects the body before calling `authorize()`, now reading the parsed/typed result. Verified live against all 5 real callers. **IPD** (`app/api/ipd/route.ts`): `lib/validation/ipd.ts`, one schema per `type` branch (bed/transfer/vitals/escalate/admission) rather than a single `z.discriminatedUnion`, since the "admission" branch is selected by the *absence* of a `type` key (defaults to `"admission"`) which a discriminated union can't match on — mirrors the route's existing if/else-if control flow exactly, just replacing each branch's `typeof` extraction with `schema.safeParse`. `authorize()` moved inside each branch (was one call before all branching) so the resource-to-check can be read from parsed data — same net permission per type, just decided after parsing instead of before. Verified live: admit-patient form, escalate button, bed status, transfer, mark-for-discharge and vitals logging all confirmed via the real `AdminIpdBeds`/`BedWardMap` UI or exact-shape API calls. **Follow-up noted, not fixed here** (out of this slice's scope): live verification surfaced that `lib/audit-diff.ts`'s default redact list treats any field literally named `token` as a secret, which incorrectly redacts `IpdAdmission.token` (a public queue number like "MGM-005", not a secret) in every IPD audit event's change-set — cosmetic/informational only, not a security issue, but worth a small `computeAuditChanges` option to un-redact specific field names per call. 59 of 62 route files remain; continue module-by-module, always with a live UI check per the `forceNew` lesson | Migrate route bodies onto shared Zod schemas module-by-module (P5 "validate everything", one validation culture) | Med | M | — |
