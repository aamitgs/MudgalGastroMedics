...
| # | Item | Current state | Build | Pri | Effort | Depends on |
|---|---|---|---|---|---|---|
| 3.1 | **Shared enterprise DataTable** | ✅ Primitive built. ✅ **Patients**, ✅ **Appointments**, ✅ **Laboratory**, ✅ **OPD Queue**, ✅ **Pharmacy**, ✅ **IPD & Beds**, ✅ **Inventory**, ✅ **HR**, ✅ **Finance**, ✅ **Procedures**, ✅ **Communication**, ✅ **CMS**, ✅ **Doctor Workflow**, ✅ **Billing Summary**, ✅ **AI Reviews**, ✅ **Audit Log**, ✅ **Automation** adopted (`lib/*-query.ts` per module). HR, Finance and Billing Summary are client-paginated since another control on the same screen needs the complete unfiltered data regardless. Automation is server-paginated like Lab (stats tiles computed server-side from the full task list, decoupled from the paginated page, same as Lab's `stats`), replacing fully-unbounded rendering (no cap at all, unlike the other hard-capped modules) with real pagination; priority sorts by clinical urgency rank (Urgent>High>Normal>Low), not alphabetically; defaults to soonest-due-first to match the original card list's order; every editable field (priority/status/due date/owner) is short enough to live inline in a column, so no detail panel was needed. **Evaluated and skipped for 3.1: Reports** (single aggregate daily-summary object, not a list of records — same reasoning as Enterprise Modules) **and User Management/Access** (deferred, not skipped — it is a genuine small list of staff accounts, but every row action is a security-sensitive RBAC operation gated by `disabled={!isSuperAdmin}` and a two-person-rule approval flow; low pagination/volume payoff doesn't clearly justify touching it without being asked). 9 modules still card-stacks | Adopt module-by-module: next candidates are Production Readiness or Settings — evaluate each for genuine tabular fit first | **High** | L | — |
