...
| # | Item | Current state | Build | Pri | Effort | Depends on |
|---|---|---|---|---|---|---|
| 3.1 | **Shared enterprise DataTable** | ✅ Primitive built. ✅ **Patients**, ✅ **Appointments**, ✅ **Laboratory**, ✅ **OPD Queue**, ✅ **Pharmacy**, ✅ **IPD & Beds**, ✅ **Inventory**, ✅ **HR**, ✅ **Finance** adopted (`lib/*-query.ts` per module). HR and Finance are each two independent DataTables on one screen (HR: Staff Directory + Attendance; Finance: Insurance Claims + Account Entries/Cashbook), both client-paginated like IPD since another control on the same screen (HR's "Mark attendance" dropdown/"Present Today" stat; Finance's claim-creation form's admission/visit dropdowns) needs the complete unfiltered arrays regardless. Finance's Claims table gained patient-drawer integration; neither Finance table needed a detail panel since the original UI never exposed extra editable fields beyond inline claim status. 17 modules still card-stacks | Adopt module-by-module: next candidates are Procedures or Communication | **High** | L | — |
