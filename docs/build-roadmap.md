...
| 1.3 | **Adopt the shared `Button`** | IN PROGRESS, updated 2026-07-08: **important scope correction** — `ActionButton` is documented as the *staff*-facing button ("the app's own button language... source of truth for staff action buttons") and has never been used on any public page. The original 63-raw-button count wrongly swept in public/patient-facing files (`PatientPortalAccess.tsx`, `PatientHealthDashboard.tsx`, `GalleryGrid.tsx`, `Header.tsx`, `AppointmentForm.tsx`) that are **out of scope** — migrating them would blur the "two products, never mixed" boundary the public site and OS/admin chrome are built on. Done so far: `DoctorPortalWorkspace.tsx` (9 → 2), `AccessLogin.tsx` (6 → 2, all 4 `buttonClass` submit buttons across the credentials/password-change/MFA-setup/MFA-verify steps now share `ActionButton variant="primary"`; the now-dead `buttonClass` constant removed), and `StaffChrome.tsx` (3 → 0: the header's search-palette trigger, dark-mode toggle and sign-out button were an exact hand-rolled duplicate — `border border-line ... text-muted ... hover:border-brand hover:text-brand` — repeated identically 3 times in this one file and again in `NotificationCenter.tsx`'s bell trigger; added a new **`outline` variant** to `ActionButton` carrying that exact class string rather than forcing an ill-fitting existing variant, so this is a zero-visual-change de-duplication, not a redesign). `WorkspaceLauncher.tsx` evaluated and **excluded**: its "← All workspaces" link is a bare text link (no `ActionButton` "link" variant exists yet), its workspace tiles are custom multi-line card selectors (don't fit the pill-button shape, same reasoning as the DoctorPortalWorkspace/queue-card exclusions), and its "Continue" button's `bg-brand`/`hover:bg-brand-dark` styling was confirmed via grep to be a deliberate public-site-token choice for this gateway page (shared only with `Footer.tsx`, `app/life-at-mgm/page.tsx`, `app/contact/page.tsx`), not a hand-rolled `ActionButton` duplicate. In DoctorPortalWorkspace/AccessLogin, 2 raw buttons remain deliberately unmigrated per file: card/list-item selectors, and in AccessLogin a bare icon-only password-visibility toggle plus a bare text-link "Forgot password?" toggle. Remaining genuinely-staff candidates: `HospitalOperatingSystem.tsx` (4, handle carefully — the one sanctioned monolith), `AdminPatients.tsx` (3) | Migrate remaining staff-facing raw buttons module-by-module; leave public-facing components on their own button styling | High | M | — |
| 1.4 | **Finish empty states** | PARTIAL, re-measured 2026-07-07: the 18 Track-3.1-converted `AdminX.tsx` modules get real empty states for free via `DataTable`'s `emptyState` prop (which renders `ModuleEmptyState` internally) — a naive "does `AdminX.tsx` import `ModuleEmptyState`" grep shows 0/26 and undercounts this badly. The 5 non-DataTable dashboards (Reports/Analytics/Enterprise Modules/Production Readiness/Settings) genuinely have none, since they don't render lists | Bring the 5 remaining dashboards to a real empty/error state where meaningful | High | M | 1.3 |
...
| 1.6 | **Audit before/after + device** | PARTIAL (~15-20%), re-measured 2026-07-07: the infra (`lib/audit-diff.ts`'s `computeAuditChanges`, `AuditChangeSet`/`AuditDeviceContext` types) is built and wired into `recordAuditEvent`, but of 38 route files calling `recordAuditEvent`, only `app/api/access/users/route.ts` actually passes `before`/`after`/`device` — every other sampled route (patients, hms, lab, auth/*) still just spreads `auditRequestMetadata()` into generic `metadata`. Narrower than the original "Patients/Prescriptions/Billing/Beds" scope — currently just the one route | Add `previousValue`/`newValue`/`device` to the audit event on create/edit/delete of Patients, Prescriptions, Billing, Beds (P4/P5) | Med | M | — |
...
| 1.12 | **Zod convergence on REST routes** | Confirmed on 2026-07-07 audit: `lib/validation/` has exactly one file (`hospital-os.ts`); 0 of 62 `app/api/*/route.ts` files import it or `zod` at all — hand-rolled `typeof` guards remain the norm everywhere | Migrate route bodies onto shared Zod schemas module-by-module (P5 "validate everything", one validation culture) | Med | M | — |
