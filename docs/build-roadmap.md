...
| # | Item | Current state | Build | Pri | Effort | Depends on |
|---|---|---|---|---|---|---|
| 3.1 | **Shared enterprise DataTable** | ✅ Primitive built. ✅ **Patients**, ✅ **Appointments**, ✅ **Laboratory**, ✅ **OPD Queue**, ✅ **Pharmacy**, ✅ **IPD & Beds**, ✅ **Inventory**, ✅ **HR**, ✅ **Finance**, ✅ **Procedures** adopted (`lib/*-query.ts` per module). HR and Finance are each two independent DataTables on one screen, both client-paginated since another control on the same screen needs the complete unfiltered arrays regardless. Procedures is server-paginated like Lab/OPD/Pharmacy (`app/api/procedures/schedule` now takes page/sortBy/sortDir/q/status, backward compatible with the old no-param full-list shape) with a "Manage" detail panel below the table for the 8-toggle safety checklist, doctor/anesthesia fields and findings/complications/notes — too much per-row surface for a table cell, same reasoning as Lab's result entry. 16 modules still card-stacks | Adopt module-by-module: next candidates are Communication or CMS | **High** | L | — |
