...
| # | Item | Current state | Build | Pri | Effort | Depends on |
|---|---|---|---|---|---|---|
| 3.1 | **Shared enterprise DataTable** | ✅ Primitive built. ✅ **Patients**, ✅ **Appointments**, ✅ **Laboratory**, ✅ **OPD Queue**, ✅ **Pharmacy**, ✅ **IPD & Beds**, ✅ **Inventory**, ✅ **HR**, ✅ **Finance**, ✅ **Procedures**, ✅ **Communication**, ✅ **CMS**, ✅ **Doctor Workflow**, ✅ **Billing Summary** adopted (`lib/*-query.ts` per module). HR, Finance and Billing Summary are client-paginated since another control on the same screen (dropdowns; stat tiles + payment-method split needing aggregate totals over the full array) needs the complete unfiltered data regardless. Billing Summary gets its own `lib/billing-query.ts` rather than reusing `lib/opd-query.ts` (same OpdVisit records as OPD Queue/Doctor Workflow) since billing filtering/sorting (billingStatus, amount) is a different vocabulary than clinical-status filtering; defaults its status filter to "Paid" to preserve the original "recent paid receipts" view. A synthetic `amount` column (derived from `estimatedAmount` via `accessorFn`, not a real field) needed an explicit `accessorFn` for TanStack's `getCanSort()` to recognize it as sortable — an `id`-only column with no accessor is silently unsortable even with `enableSorting` left at its true default. 12 modules still card-stacks | Adopt module-by-module: next candidates are AI Reviews or Enterprise Modules | **High** | L | — |
