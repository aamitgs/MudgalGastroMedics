...
| 1.12 | **Zod convergence on REST routes** | IN PROGRESS, started 2026-07-08. Four slices done, all with a shared `lib/validation/http.ts` (`firstZodIssueMessage`, mapping a `ZodError` to the single string the `{ ok, error }` REST shape expects). **Patients** (`app/api/patients/route.ts` + `lib/validation/patients.ts`) — **bug caught by live verification before shipping**: the real `AdminPatients.tsx` client sends `forceNew: duplicateMatch && confirmedNewPatient`, which is JS `null` (not `false`) whenever there's no duplicate match; `.optional()` rejected that with a 400 on every ordinary create, fixed with `.nullish()`. This is why every slice needs a real browser run against the actual calling component(s), not hand-picked direct API payloads. **OPD** (`app/api/opd/route.ts` + `lib/validation/opd.ts`) — the field-level authorization split (clinical → `prescriptions`, billing → `billing`, else `appointments`) still inspects the body before calling `authorize()`, now reading the parsed/typed result; verified against all 5 real callers. **IPD** (`app/api/ipd/route.ts` + `lib/validation/ipd.ts`) — one schema per `type` branch (bed/transfer/vitals/escalate/admission) rather than a single `z.discriminatedUnion`, since the "admission" branch is selected by the *absence* of a `type` key, which a discriminated union can't match on; `authorize()` moved inside each branch so the resource-to-check reads from parsed data. **Follow-up noted, not fixed** (out of scope for that slice): `lib/audit-diff.ts`'s default redact list treats any field named `token` as a secret, incorrectly redacting `IpdAdmission.token` (a public queue number, not a secret) — cosmetic only. **HR** (`app/api/hr/route.ts` + `lib/validation/hr.ts`, the single largest hand-rolled file at 18 `typeof` guards) — preserved two intentionally-lenient existing behaviors as `.default(...)` rather than making them required: an invalid/missing staff `role`/`shift` and attendance `status` previously silently fell back to `"Admin"`/`"General"`/`"Present"` rather than rejecting; real caller (`AdminHR.tsx`) always sends valid values via constrained `<select>`s, so this only matters for defensive parity, not real usage. Verified live against the real staff-create form plus attendance/status/permissions updates. 58 of 62 route files remain; continue module-by-module, always with a live UI check per the `forceNew` lesson | Migrate route bodies onto shared Zod schemas module-by-module (P5 "validate everything", one validation culture) | Med | M | — |
