...
| # | Item | Current state | Build | Pri | Effort | Depends on |
|---|---|---|---|---|---|---|
| 3.1 | **Shared enterprise DataTable** | ✅ Primitive built. ✅ **Patients**, ✅ **Appointments**, ✅ **Laboratory** adopted with server pagination/sort/filter (`lib/patient-query.ts`, `lib/appointment-query.ts`, `lib/lab-query.ts` — all backward compatible). Lab also gained a `criticalOnly` filter and an "Acknowledged by" line restoring visibility the table refactor had quietly dropped. 23 modules still card-stacks | Adopt module-by-module: **OPD Queue or Pharmacy next** (same list-with-status shape) | **High** | L | — |
