...
import { AdminCmsWorkspace } from "@/components/cms/AdminCmsWorkspace";
...
export const metadata: Metadata = {
  title: { absolute: "CMS • MudgalGastromedics OS" },
  description: "Public website content management.",
  alternates: { canonical: "/mudgalgastromedics-os/cms" },
...
export default async function CmsPage() {
  ...
  if (!canOpenModule(context, "module-cms")) {
    return (
      <main>
        <section className="border-b border-line bg-surface">
          <div className="mx-auto flex w-[min(1560px,calc(100%-32px))] flex-wrap items-center justify-between gap-3 py-5">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.14em] text-brand">CMS</p>
              <h1 className="mt-1 text-2xl font-bold leading-tight text-ink">Website content</h1>
            </div>
          </div>
        </section>
        <div className="mx-auto grid w-[min(1560px,calc(100%-32px))] place-items-center py-16">
          <div className="grid max-w-md gap-3 rounded border border-line/80 bg-white p-8 text-center shadow-sm">
            ...
            <p className="text-sm leading-relaxed text-muted">Ask a Super Admin if you need CMS access.</p>
      ...
      <section className="border-b border-line bg-surface">
        <div className="mx-auto flex w-[min(1560px,calc(100%-32px))] flex-wrap items-center justify-between gap-3 py-5">
          ...
          <p className="max-w-xl text-sm leading-relaxed text-muted">
            Publish and manage content shown on the public hospital website.
          </p>
        </div>
      </section>
