...
export const metadata: Metadata = {
  title: { absolute: "Inventory • MudgalGastromedics OS" },
  description: "Stock levels, reorder thresholds and purchase orders.",
  alternates: { canonical: "/mudgalgastromedics-os/inventory" },
...
export default async function InventoryPage() {
  ...
  if (!canOpenModule(context, "module-inventory")) {
    return (
      <main>
        <section className="border-b border-line bg-surface">
          <div className="mx-auto flex w-[min(1560px,calc(100%-32px))] flex-wrap items-center justify-between gap-3 py-5">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.14em] text-brand">Inventory</p>
              <h1 className="mt-1 text-2xl font-bold leading-tight text-ink">Stock &amp; purchase orders</h1>
            </div>
          </div>
        </section>
        <div className="mx-auto grid w-[min(1560px,calc(100%-32px))] place-items-center py-16">
          <div className="grid max-w-md gap-3 rounded border border-line/80 bg-white p-8 text-center shadow-sm">
            ...
            <p className="text-sm leading-relaxed text-muted">Ask a Super Admin if you need Inventory access.</p>
      ...
      <div className="mx-auto grid w-full max-w-[1560px] gap-5 px-4 py-5 lg:px-6">
        <div className="rounded border border-line bg-surface p-5">
          ...
          <p className="mt-2 max-w-xl text-sm leading-relaxed text-muted">
            Stock levels, reorder thresholds, vendors and purchase orders.
          </p>
        </div>
        <AdminInventory />
      </div>
