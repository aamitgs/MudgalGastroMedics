...
import { AdminEnterpriseModules } from "@/components/enterprise/AdminEnterpriseModules";
...
export const metadata: Metadata = {
  title: { absolute: "Modules • MudgalGastromedics OS" },
  description: "Internal build tracker for enterprise/HMS modules.",
  alternates: { canonical: "/mudgalgastromedics-os/modules" },
...
export default async function ModulesPage() {
  ...
  if (!canOpenModule(context, "module-modules")) {
    return (
      <main>
        <section className="border-b border-line bg-surface">
          <div className="mx-auto flex w-[min(1560px,calc(100%-32px))] flex-wrap items-center justify-between gap-3 py-5">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.14em] text-brand">Modules</p>
              <h1 className="mt-1 text-2xl font-bold leading-tight text-ink">Build tracker</h1>
            </div>
          </div>
        </section>
        <div className="mx-auto grid w-[min(1560px,calc(100%-32px))] place-items-center py-16">
          <div className="grid max-w-md gap-3 rounded border border-line/80 bg-white p-8 text-center shadow-sm">
            ...
            <p className="text-sm leading-relaxed text-muted">Ask a Super Admin if you need Modules access.</p>
      ...
      <section className="border-b border-line bg-surface">
        <div className="mx-auto flex w-[min(1560px,calc(100%-32px))] flex-wrap items-center justify-between gap-3 py-5">
          ...
          <p className="max-w-xl text-sm leading-relaxed text-muted">
            Internal tracker for hospital system build status.
          </p>
        </div>
      </section>
