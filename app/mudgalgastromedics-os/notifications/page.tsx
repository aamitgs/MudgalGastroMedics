...
import { NotificationsPage as NotificationsPageContent } from "@/components/hospital-os/NotificationsPage";
...
export const metadata: Metadata = {
  title: { absolute: "Notifications • MudgalGastromedics OS" },
  description: "Hospital-wide operational alerts, escalations and staff announcements.",
  alternates: { canonical: "/mudgalgastromedics-os/notifications" },
  robots: { index: false, follow: false }
};

/** Sidebar's "Notifications" entry (Dashboard/Notifications/Patients routing fix) — same page.tsx pattern as staff-notes/page.tsx. */
export default async function NotificationsPage() {
  ...
  if (!canOpenModule(context, "module-notifications")) {
    return (
      <main>
        <section className="border-b border-line bg-surface">
          <div className="mx-auto flex w-[min(1560px,calc(100%-32px))] flex-wrap items-center justify-between gap-3 py-5">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.14em] text-brand">Notifications</p>
              <h1 className="mt-1 text-2xl font-bold leading-tight text-ink">Staff notification inbox</h1>
            </div>
          </div>
        </section>
        <div className="mx-auto grid w-[min(1560px,calc(100%-32px))] place-items-center py-16">
          <div className="grid max-w-md gap-3 rounded border border-line/80 bg-white p-8 text-center shadow-sm">
            ...
            <p className="text-sm leading-relaxed text-muted">Ask a Super Admin if you need Notifications access.</p>
      ...
      <div className="mx-auto grid w-full max-w-[1560px] grid-cols-[minmax(0,1fr)] gap-5 px-4 py-5 lg:px-6">
        <div className="rounded border border-line bg-surface p-5">
          ...
          <p className="mt-2 max-w-xl text-sm leading-relaxed text-muted">
            Operational alerts, escalations and hospital-wide announcements — the same inbox as the bell icon, in full.
          </p>
        </div>
